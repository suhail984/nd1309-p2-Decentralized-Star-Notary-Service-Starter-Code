...
it('lets 2 users exchange stars', async() => {
    ...
    assert.notEqaul(owner1BeforExchange,owner1AfterExchange, "Oner of token1 not changes");
    assert.notEqaul(owner2BeforExchange,owner2AfterExchange, "Oner of token2 not changes");
...
it('lets a user transfer a star', async() => {
    ...
    let user2 = accounts[2];
    let starId = 9;
    await instance.createStar('user1 star', star1, {from:user1});
    let ownerBeforeTransfer = (await instance.ownerOf.call(starId));
    await instance.tranafer(user2,starId,{from:user1});
    let ownerAfterTransfer = (await instance.ownerOf.call(starId));
    assert.notEqaul(ownerBeforeTransfer, ownerAfterTransfer, "Token is trnasfered")
});
